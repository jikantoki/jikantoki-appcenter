  ...
  .profile-zone(
    ...
    //-.createdat.mx-2(v-show="!loading")
      p(
        ...
        ) {{ new Date(userData.createdAt * 1000) }}からENOKI Appcenterを利用しています
...
export default {
  ...
  async mounted() {
    ...
    setTimeout(() => {
      ...
      if (!ctx) return false
      QRCode.toCanvas(canvas, this.myLink, {
        scale: 10,
      })
  ...
  methods: {
    ...
    async friendRequest(userId: string) {
      ...
      /** サーバーに送信 */
      try {
        const res = await this.sendAjaxWithAuth('/friendRequest.php', {
        ...
        switch (res.body.status) {
          case 'request':
          case 'ok': {
            this.followDialogMessage =
              '友達申請が完了しました！相手の承認をお待ちください'
            break
          }
          case 'cannot': {
